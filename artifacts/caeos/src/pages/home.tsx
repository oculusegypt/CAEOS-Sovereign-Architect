    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <section className="container mx-auto px-6 py-24 text-center">
        <div className="mx-auto max-w-4xl animate-fade-in">
          ...
          <p className="mb-4 text-xl text-[#0ea5e9] md:text-2xl">
            {t("hero.subtitle")}
          </p>
          ...
          <div className={`flex flex-col items-center justify-center gap-4 sm:flex-row ${dir === "rtl" ? "sm:flex-row-reverse" : ""}`}>
            <Link
              href="/wizard"
              className="group flex items-center gap-2 rounded-lg bg-[#2563eb] px-8 py-4 font-semibold text-white transition-all hover:bg-blue-600"
            >
              {t("hero.cta_primary")}
              <ChevronRight className={`h-5 w-5 transition-transform group-hover:translate-x-1 ${dir === "rtl" ? "rotate-180" : ""}`} />
            </Link>
            <a
      ...
      <section className="container mx-auto px-6 py-20">
        <div className="grid gap-8 md:grid-cols-3">
          ...
          />
          <FeatureCard
            icon={<GitBranch className="h-8 w-8 text-[#10b981]" />}
          ...
          />
          <FeatureCard
            icon={<Scale className="h-8 w-8 text-[#f59e0b]" />}
      ...
      <footer className="border-t border-white/10 bg-slate-900/50 px-6 py-8">
        <div className="container mx-auto flex flex-col items-center justify-between gap-4 md:flex-row">
          ...
          <div className="flex items-center gap-4 text-sm text-slate-400">
            ...
            <span className="flex items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-[#0ea5e9] animate-pulse-soft" />
...
function FeatureCard({
  ...
  return (
    <div className="animate-slide-up rounded-xl border border-white/10 bg-white/5 p-8 backdrop-blur-sm transition-all hover:border-white/20 hover:bg-white/10">
      <div className="mb-4">{icon}</div>
      <h3 className="mb-2 text-xl font-semibold">{title}</h3>
      <p className="mb-4 text-slate-300">{description}</p>
      <div className="border-t border-white/10 pt-4">
        <h4 className="mb-1 text-lg font-semibold text-[#0ea5e9]">{titleAr}</h4>
        <p className="text-sm text-slate-400" dir="rtl">{descriptionAr}</p>
      </div>
    </div>
  );
}
